...
# Auswahlfeld
selected = st.radio("Wähle die richtige Antwort:", antworten, index=st.session_state["lernkontrolle_selected"] if st.session_state["lernkontrolle_selected"] is not None else 0)
...
if st.session_state["lernkontrolle_abgegeben"]:
    if st.session_state["lernkontrolle_feedback"] == "richtig":
        ...
        if st.button("Weiter"):
            # Hier kannst du auf die nächste Seite springen, z.B.:
